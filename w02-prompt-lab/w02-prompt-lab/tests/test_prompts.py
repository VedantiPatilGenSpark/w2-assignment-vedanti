...
def test_extract_v2_uses_fictional_few_shots_not_case_documents() -> None:
    v2_text = _normalize(_read_text(V2_PATH))
    assert "Northglass Merchant Review Standard" in v2_text
    assert "Alder Quay Small Business Review Policy" in v2_text
    assert EXAMPLE_OUTPUT_MARKER in v2_text
